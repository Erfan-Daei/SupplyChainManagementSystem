...
{
    public class RequestChangePasswordService : IRequestChangePassword
    {
        ...
        public RequestChangePasswordService(IUserRepository_Query user_Query
        ...
        public async Task<ResultDto> RequestChangePasswordAsync(RequestChangePasswordCommandRequest request, CancellationToken ct)
        {
            try
            {
                ...
                //hashed password temporarly saved in database
                var hashedPassword = _hashManager.BCryptHashPassword(request.Password);

                var tempHashedPasswordToken = UserToken.Create
                    (
                        hashedPassword,
                        UserTokenType.TepmHashedPassword.ToString(),
                ...
                var hashedConfirmationToken = UserToken.Create
                    (
                        hashedDigit,
                        UserTokenType.ChangePasswordConfirmation.ToString(),
                        _changePasswordConfirmationSettings.UserTokenExpireMinutes,
                        user.UserId
                    );

                await _user_Command.DeleteChangePasswordTokensAsync(user.UserId);

                await _user_Command.AddChangePasswordTokensAsync(new List<UserToken>
                {
                    tempHashedPasswordToken,
